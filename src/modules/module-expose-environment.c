...
static const char *cache_env(struct userdata *u, pa_client *c, const char *name) {
    ...
    /*
     * We hash by the client index instead of the client pointer here for two reasons:
     *
     * 1) In principle the monotonically increasing index should provide a better hash distribution
     *    than the pointer.
     *
     * 2) Even if we fail to correctly evict the cache when a client is gone, the index is guaranteed
     *    not to be reused before it wraps over so we should not get incorrect cache hits because of
     *    lingering entries. Nothing like that can be guaranteed for the pointer, which might get
     *    reused by first getting freed then handed out when a new client is allocated.
     */
...
int pa__init(pa_module *m) {
    ...
    u->cache = pa_hashmap_new_full(pa_idxset_trivial_hash_func, pa_idxset_trivial_compare_func,
                                   NULL, (pa_free_cb_t)pa_hashmap_free);
